...
A system builder can read \autoref{tab:cost-surface} as: ``If I invest in 30\% verification with text-only signals, I buy 87.6\% accuracy. If I instead export entropy and use that for triage, I buy 90.9\% for a bit more overhead. If I compose tensor with bounded lookup, I buy 91.5\%.'' Text-guided triage hits a ceiling around 87--88\% because length is a weak signal under fair comparison; the tensor advantage (+3--5pp) is consistent and architecturally driven. The composed judge's additional lift comes from handling failure modes that entropy alone cannot catch.
