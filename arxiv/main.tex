...
text---no bounded supervisor can distinguish epistemically honest systems from
systems that fabricate plausibly (Representational Impossibility,
Learnability Impossibility). The impossibility holds regardless of model scale
or training procedure, including RLHF and instruction tuning.
...
entropy, attention summaries, and log-probability distributions. Under
length-controlled evaluation where no text feature exceeds AUC 0.70, per-token
entropy achieves AUC 0.757 and outperforms text baselines by 2.5--3.9
percentage points at every budget level tested (10\%, 20\%, 30\%). The entropy
signal generalizes across architectures (Spearman $\rho = 0.762$).
...
\begin{itemize}
    \item \textbf{Response length}: Fabrications tend to be verbose while factual retrieval is concise. Under length-controlled evaluation (where we control for this asymmetry), AUC dropped to 0.63.
    ...
    \item \textbf{Citations}: Fabrications often include citations that sound real. Checking citation inclusion is unreliable---false citations are as fluent as real ones.
    \item \textbf{Composed judges}: Combining multiple text features with machine learning. Under length control, pooled AUC never exceeded 0.70.
\end{itemize}
...
\subsection{The FLP Connection: An Impossibility Framework}
...
\subsection{Why This Explains Self-Report Inversion}
...
\begin{itemize}
    \item \textbf{Per-token entropy}: How uncertain was the model while generating this token?
    \item \textbf{Attention geometry}: Were the attention patterns coherent across layers?
    \item \textbf{Log-probabilities}: What probability did the model assign to the generated tokens?
\end{itemize}
...
grammar; scaling solved coherence~\citep{kaplan2020scaling, wei2022emergent}; scaling will solve fabrication. Lin et
al.\ tested this assumption directly and found an inverse scaling trend: larger
models were \emph{less} truthful on TruthfulQA~\citep{lin2022truthfulqa}. The
...
fabricating, but the interface provides no channel through which a supervisor
can verify this knowledge independently.
...
here. This boundary is a design choice, not a fixed property of current
systems (\autoref{sec:discussion}).
\end{remark}
...
supervisor must evaluate outputs under time and cost constraints.
Training-time data curation (e.g., RLHF annotation) operates under a different
cost regime: human annotators may spend unbounded time per example. The
impossibility applies when verification cost exceeds the supervisor's
per-query budget at inference time, not to offline dataset construction.
\end{remark}
...
\begin{enumerate}
    \item \textbf{Per-token entropy}: For each token in the generated sequence, the entropy of the probability distribution over the next token. Low entropy means the model was decisive; high entropy means uncertain.

    \item \textbf{Attention summaries}: Statistical measures of attention coherence across layers. A coherent generation has consistent attention patterns; incoherent generations (fabrications with internal contradictions) show fragmented attention.

    \item \textbf{Log-probabilities}: The probability the model assigned to each token actually generated. A confident (plausible) output has high per-token probabilities; uncertain outputs have lower probabilities.
...
\textbf{Cost}: Exporting per-token entropy adds ${\sim}2.4\%$ overhead; the full signal set adds ${\sim}7.1\%$ overhead. This is measurement, not additional forward passes.
...
\textbf{Key finding: Tensor-guided verification outperforms the text baseline at every budget level by +2.5--3.9 percentage points.}
...
\begin{itemize}
    \item OLMo-3: 69.5\% $\to$ 75.1\% at 10\% budget
    \item Llama-3.1: 82.5\% $\to$ 88.1\%
    \item Qwen3: 87.0\% $\to$ 91.7\%
    \item Mistral: 64.0\% $\to$ 72.0\%
\end{itemize}

Cross-model entropy agreement: Spearman $\rho = 0.762$ (rank-order correlation where 1.0 is perfect agreement). This high agreement suggests the signal is driven by query properties (is this query answerable?) rather than model artifacts.
...
\subsection{Why Tensor Wins: The Length Control}
...
\subsection{Complementary Failure Modes: Why Composition Helps}\label{sec:eval_bounded}
...
\textbf{The inversion pattern appears in citations}: Fabricated citations are generated more confidently than real ones because the model can fluently invent a plausible citation format (Smith et al., 2024) without retrieving actual bibliographic details. Real citations require looking up specific information, which introduces uncertainty.

So entropy \emph{inverts} for citations: low entropy on fabricated citations, higher entropy on real ones. This is exactly backwards from what we need.
...
\subsection{Signal Access as a Policy Decision}
...
\begin{itemize}
    \item Early completion APIs exposed log-probabilities
    \item Recent reasoning models often don't
    \item Proprietary models may retain exclusive access
\end{itemize}
...
\begin{itemize}
    \item If you have access to log-probabilities and entropy, you can build multi-tiered verification.
    \item If you don't, you're limited to text-only approaches, which the impossibility results bound.
\end{itemize}

\textbf{Policy implication}: Responsibility concentration. Who bears the cost of verification? Is that cost visible or hidden?

A provider could:
\begin{itemize}
    \item Export full epistemic telemetry (highest transparency, enables external verification)
    \item Export aggregated signals (moderate transparency, verification is guided but external)
    \item Export nothing beyond text (zero transparency, all verification responsibility falls on the provider)
\end{itemize}
...
\subsection{Domain-Specific Failure Modes}
...
\fakepara{Citations.} Fabricated citations are generated fluently (low entropy) because the model invents plausible formats without retrieving specifics. Real citations require looking up details, introducing uncertainty (higher entropy). Entropy inverts on citation queries.

\fakepara{Domain calibration.} In domains where confident answers are rare or where refusals are common, entropy thresholds need adjustment. The cost surface in \autoref{sec:eval} is calibrated to a balanced query set; in real deployment, base rates vary by domain.

Practitioners should:
\begin{itemize}
    \item Validate entropy thresholds on domain-specific held-out data
    \item Use composed judges where entropy fails (e.g., citation lookup for citation queries)
    \item Monitor for drift as models improve or change
\end{itemize}
...
We constructed a tensor interface that escapes the impossibility by exporting signals the model cannot independently control. Per-token entropy achieves AUC 0.757 under length-controlled evaluation, outperforming all text features. The signal generalizes across architectures (Spearman $\rho = 0.762$).
...
\begin{lstlisting}[caption=Representational Impossibility (Lean)]
theorem representational_impossibility
    {Q W : Type} {RS : ResponseSpace}
    (AC : AmbiguityCondition Q W)
    (reqs : EpistemicHonestyReqs RS)
    ((*$\pi$*) : PredictorCentricPolicy Q RS)
    (prob_sum_le_one : (*$\pi$*).prob AC.q reqs.r_corr
                     + (*$\pi$*).prob AC.q RS.bot (*$\le$*) 1)
    (honest_wA : (*$\pi$*).prob AC.q reqs.r_corr (*$\ge$*) 1 - reqs.(*$\varepsilon$*))
    (honest_wB : (*$\pi$*).prob AC.q RS.bot (*$\ge$*) 1 - reqs.(*$\varepsilon$*)) :
    False := by
  have h_sum : (*$\pi$*).prob AC.q reqs.r_corr
             + (*$\pi$*).prob AC.q RS.bot (*$\ge$*) 2 * (1 - reqs.(*$\varepsilon$*)) :=
    by linarith
  have h_gt : 2 * (1 - reqs.(*$\varepsilon$*)) > 1 :=
    by linarith [reqs.(*$\varepsilon$*)_lt_half]
  linarith
\end{lstlisting}

\subsection{Theorem 2: Learnability Impossibility}

\begin{lstlisting}[caption=Learnability Impossibility (Lean)]
theorem learnability_impossibility
    {Q W : Type} {RS : ResponseSpace} {(*$\Theta$*) : Type}
    (AC : AmbiguityCondition Q W)
    (S : BoundedSupervisor Q W RS)
    (A : LearningAlgorithm Q W RS (*$\Theta$*))
    ((*$\theta$*) : (*$\Theta$*)) :
    A.update AC.q S.r_fab S AC.wA (*$\theta$*)
    = A.update AC.q S.r_fab S AC.wB (*$\theta$*) := by
  apply A.update_depends_on_obs S AC (*$\theta$*)
  exact S.indistinguishable AC
\end{lstlisting}

\subsection{Lemma: Observation Monotonicity}

\begin{lstlisting}[caption=Observation Monotonicity (Lean)]
theorem observation_monotonicity
    ...
    Information (layer2.judge (*$\circ$*) layer1.judge)
    (*$\le$*) Information layer1.judge := by
  intro obs
  simp [Information, entropy]
  exact deterministic_function_reduces_entropy
        (layer2.judge (*$\circ$*) layer1.judge)
\end{lstlisting}

\subsection{Corollary: No Stack of Text-Only Judges Escapes}

\begin{lstlisting}[caption=Layered Judges Cannot Escape (Lean)]
theorem layered_judges_cannot_escape
    (judges : List (TextOnlySupervisorLayer Obs Obs))
    (stack : Obs (*$\to$*) Obs := judges.foldl ((*$\cdot$*)(*$\circ$*)(*$\cdot$*)) id) :
    (*$\forall$*) h (*$\in$*) PlausibleLies,
      stack(JudgeObserve h)
      = stack(JudgeObserve (correct_response h))
      (*$\lor$*) stack cannot distinguish them := by
  intro h h_in_lies
...
\midrule
\multicolumn{4}{l}{\textit{$\times$4 models = 1000 queries; deduplicates to 800 pairs}} \\
\bottomrule
\end{tabular}
