...
%\author{Tony Mason}
\author{Tony Mason\thanks{\texttt{fsgeek@\{cs.ubc.ca,gatech.edu,wamason.com\}}} \and Vaastav Anand\thanks{Max Planck Institute for Software Systems. \texttt{vaastav@mpi-sws.org}}}
...
language model components: \textbf{asking a model how confident it is buys
almost nothing.} Across four model families (OLMo-3, Llama-3.1, Qwen3,
Mistral), self-reported confidence is pinned near the top of its scale
regardless of grounding --- a mean of 0.98 on knowable queries and 0.73 on
unknowable ones, with 43\% of questions that have no correct answer receiving a
literal ``100''. The ordering is better than chance (AUC 0.66--0.92), but so
compressed that any threshold admitting most correct answers admits most
fabrications too.

We prove, under explicit formal assumptions, that this is not a capability gap but an observational one. Under
...
responses. Within our formal model, these impossibilities hold regardless of model scale or training
procedure, including RLHF and instruction tuning.

We construct a \emph{tensor interface} that escapes the impossibility by
exporting computational byproducts---per-token
entropy and log-probability distributions---that are structurally coupled to correctness under standard training. Per-token
entropy achieves pooled AUC 0.870 on that set, and 0.757 under a
length-controlled comparison that holds response length fixed; it outperforms
all text baselines by
3.3--4.6 percentage points at every budget level tested (10\%, 20\%, 30\%).
The entropy signal generalizes across architectures (Spearman $\rho = 0.762$).

The core contribution is a \emph{cost surface}---the empirical mapping from verification budget (fraction of queries receiving expensive checks) to detection accuracy for each judge strategy---a practical lookup for system builders deciding how to allocate verification resources. The contribution is the map. The
...
We start with a finding that should trouble anyone building systems with language model components: \textbf{asking a model how confident it is buys almost nothing.}

We tested this across four model families (OLMo-3, Llama-3.1, Qwen3, Mistral) by asking simple questions: ``How confident are you in this answer?'' The answer is nearly always the same one. Mean self-reported confidence is 0.98 on knowable queries and 0.73 on unknowable ones, and 43\% of the unknowable queries---questions about people, papers, and events that do not exist---are answered with a literal ``100''. Area under the ROC curve ranges from 0.66 to 0.92 across architectures, where 0.5 is random guessing: better than chance, but the distributions overlap so heavily at the ceiling that no threshold separates them usefully.

Self-reported confidence is not a lie. It is a signal with almost no dynamic range, which for a bounded supervisor amounts to the same thing.

\begin{remark}[Correction]
An earlier version of this paper reported the stronger claim that self-report is \emph{inverted}---AUC 0.28--0.36, below random---and said ``we checked multiple times.'' That was an artifact of a confidence-parsing bug: the extractor decoded the prompt along with the generation and took the first integer it found, which was usually the \texttt{0} in the prompt's own ``scale of 0-100'' text. The error was asymmetric, zeroing 47\% of knowable responses against 31\% of unknowable ones, which manufactured a clean-looking inversion. It was reported by Chris Brew and is fixed at the source; the corrected values are above. The impossibility results do not depend on inversion---they require only that self-report be uninformative to a bounded supervisor---and the entropy and budget results are unaffected.
\end{remark}
...
The standard assumption is that scaling solves this: larger models, more RLHF training, instruction tuning; eventually they'll know what they know. But Lin et al.\ found the opposite: larger models were \emph{less} truthful on TruthfulQA~\citep{lin2022truthfulqa}. And our self-report finding suggests why: the problem isn't capability, it's interface. The text-only channel through which models expose their reasoning cannot carry the signals a supervisor needs to distinguish grounded generation from confident fabrication.

\subsection{What This Paper Contributes}

We make three contributions:

\textbf{1. An impossibility result for text-only verification.} We prove, under explicit formal assumptions, that a text-only observation model---where a supervisor can see only the model's output text, not its internal computation---is architecturally insufficient to verify epistemic honesty under bounded supervision. It's not a capability gap; it's an observational gap.

\textbf{2. A tensor interface that escapes the impossibility.} We construct an epistemic observability interface that exports internal signals alongside text: per-token entropy traces and log-probability distributions. These are byproducts of inference, telemetry the model generates during computation. Under standard training, the model cannot separately tune entropy without affecting correctness, which is why entropy generalizes where text features diverge.

\textbf{3. The empirical cost surface.} We map verification effectiveness across four judge strategies (no judge, text-only, tensor-guided, composed) at three budget levels (10\%, 20\%, 30\%). Per-token entropy achieves pooled AUC 0.870 on this set (0.757 when response length is held fixed in a separate length-controlled study) and outperforms text baselines by 3.3--4.6 percentage points at every budget level. The tensor interface works. The cost surface tells system builders what each level of verification investment buys.

\subsection{Why This Matters}

Deploying language models in critical systems (medical, legal, financial) requires knowing what the model knows. The current interface doesn't give us that information. This paper shows why, and shows a path forward.
...
The model's internal computation---the attention patterns, the per-token entropy, the per-layer logits---contains information that distinguishes grounded generation from fabrication. When a model answers from what it reliably represents, generation is comparatively low-entropy; when it constructs a response without such support, entropy is higher (in our data, 0.24 nats against 0.61). The computation leaves traces.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.85\textwidth]{epistemic_trace_demo.pdf}
...
\begin{itemize}
    ...
    \item \textbf{Citations}: the entropy signal is markedly weaker on citation-like queries than elsewhere. We analyze this failure mode in detail in \autoref{sec:eval_bounded}.
...
\subsection{Query Categories and the Compression of Self-Report}

To characterize what self-report does and does not carry, we test five types of queries:
\begin{itemize}
...
Self-reported confidence orders these classes in the right direction, but barely uses its scale. Knowable queries average 0.98 and unknowable ones 0.73; the fabricated-entity queries (Glavinsky, Westphalia) average 0.75, and only the Private/Future class---where the model has been trained to refuse---drops appreciably, to 0.60. Nearly half of all unknowable queries still draw a literal ``100''.

Self-reported confidence should be a signal of epistemic honesty: high confidence on what the model knows, low on what it does not. What the text channel actually delivers is a number that is almost always near its maximum, because the model is optimized for \emph{coherent text generation} rather than honest metacognition.

A model optimized for coherence learns to generate fluent, confident output. Fabrications are fluent and confident by construction: they are ungrounded text that reads smoothly. Grounded generation of obscure facts may be no more assertive, and sometimes less. The confidence report is generated by the same process that generates the answer, so it inherits the answer's fluency rather than measuring its grounding.

There is no training signal that would cause a text-only model to pair \emph{accurate confidence estimates} with \emph{accurate generation}. The text channel does not expose the information needed to learn the distinction. The model learns to be confident, which is good for fluency, and the residual correlation with grounding is too weak to threshold on.

Instruction tuning does not fix this. Models trained to follow instructions learn to express confidence convincingly, producing fluent and confident text regardless of whether the answer is grounded. They become better at generating confident-sounding text, not at knowing what they know.

\subsection{The Escape Condition}

If text-only observation is insufficient, what observation model \emph{is} sufficient?

The answer: \textbf{export signals that, under standard training, the model cannot independently control} (see \autoref{sec:design} for the formal argument and caveats).

Text is fully controllable through training. A model learns to sound confident, to match citation formats, to hedge appropriately, all while fabricating. But the model's computation is different: per-token entropy, log-probabilities, and attention patterns are byproducts of inference, structurally coupled to what the model actually computes. This is the tensor interface: export the computation's telemetry alongside the text. \autoref{sec:design} develops the formal argument for why these signals resist manipulation.
...
Why is self-report so uninformative? Why don't more supervisors help? Why doesn't instruction tuning fix it?

The answer is that epistemic honesty is not a capability problem. As noted in the introduction, scaling has not resolved it. The theorems that follow explain why. It's not about what the model knows; it's about what the model can \emph{communicate}. Text-only observation lacks the degrees of freedom to express different epistemic states. The model may know it is fabricating. But the interface provides no channel to communicate this knowledge independently.
...
here. This boundary is a design choice, not a technological constraint. The industry trend is toward \emph{greater} opacity, not less; we analyze the governance implications in \autoref{sec:discussion}.
\end{remark}
...
\subsection{The RAG Escape?}\label{sec:rag}
...
It does not, and the reason is worth separating from the one usually given.
Retrieval does not give the policy access to $w$. It gives access to a
document corpus $w_{docs}$ --- a distinct world state, assembled at some
earlier time, which may be stale, incomplete, or simply wrong about $w$. A
retrieval-augmented system can therefore be perfectly faithful to its index
and still fabricate about the world, because faithfulness to documents is not
grounding in reality. Retrieval relocates the epistemic question from the
model to the corpus rather than answering it, and the supervisor's problem
reappears one level down: verifying that $w_{docs}$ agrees with $w$ is itself
a verification task under a bounded budget.

\begin{definition}[Grounded Policy]
A grounded policy is a stochastic function $\pi: \Q \times \D \to \Delta(\R)$,
where $\D$ represents documents retrieved from a corpus at inference time.
Note that $\D$ is drawn from $w_{docs}$, not from $w$; the distinction is the
subject of the preceding paragraph. This policy class \emph{can} theoretically represent different
...
supervisor $S$ can \emph{guarantee} convergence to a Grounded Policy that
satisfies Epistemic Honesty across both contexts $w_A$ and $w_B$ for the
response $r_{fab}$: the supervisor's signal is identical in the two worlds, so
nothing in the training process drives the policy toward the required split.
\end{theorem}
...
\item Identical updates under identical observations give the optimizer no
  information to differentiate the worlds. Whatever the algorithm converges to,
  the training signal did not distinguish $w_A$ from $w_B$ on this response.
\item The split behavior required for honesty therefore cannot be
  \emph{learned from this supervisor}. \qedhere
\end{enumerate}
\end{proof}

\begin{remark}[Empirical Bias]
The theorem constrains what the supervisor's signal can teach, not what a
system can do by other means: a policy that reads retrieved documents at
inference may still split correctly, having acquired the distinction from $D$
rather than from training (\autoref{sec:rag}). What training cannot do is
supply that distinction. Empirically, the tie is not left open --- observation
shows a strong bias. Supervisors typically prefer
...
The answer is not to build new models or change training procedures. It's to export signals the model generates anyway: internal telemetry that is structurally coupled to what the model computes.
...
The core property: \textbf{Under standard training, the model cannot independently tune these signals without affecting correctness.} Whether adversarial fine-tuning could break this coupling is an open question (\autoref{sec:discussion}).
...
This is why entropy generalizes across architectures while text features diverge (\autoref{sec:eval}). Entropy is architectural; text features are behavioral.
...
\begin{itemize}
    ...
    \item \textbf{Provenance Binding}: For citation queries, a bounded lookup judge would check whether cited sources exist --- a tier of verification orthogonal to entropy-based triage. We model this tier rather than implement it (\autoref{sec:eval_bounded}).
\end{itemize}
...
\textbf{Scope of empirical evaluation.} Our experiments primarily evaluate Verification Independence (entropy as a triage signal). State Exteriority is present by construction (the query set has known ground truth) but is not independently varied. Provenance Binding is not implemented; for the citation sub-category we substitute an oracle to bound what such a judge could buy (\autoref{sec:eval_bounded}). Independent empirical evaluation of State Exteriority and Provenance Binding across diverse deployment conditions is future work.
...
Because different failure modes require different signals. The impossibility says you cannot use text alone. It doesn't say one signal suffices for everything. Entropy weakens on citations; bounded lookup fails on open-ended questions (no external source to check). Composition allocates a verification budget across failure classes rather than spreading it uniformly.
...
The tensor interface is not a fundamental property of transformers; it's a choice about what to export. Access to internal signals has eroded as models have scaled (see \autoref{sec:discussion} for the governance implications). A system builder deploying a closed-weight model without access to entropy or log-probabilities is limited to text-only approaches, which the impossibility results constrain.
...
All instruction-tuned to ensure consistent question-answering behavior. Note: Qwen3 4B-Instruct also serves as the Tier~2 LLM classifier for ground truth evaluation (a different task: factual verification, not response generation). The 93.8\% human-agreement rate on blinded samples provides independent cross-validation of this dual role.
...
\begin{enumerate}
    ...
    \item \textbf{Text-guided judge using response length} (78.4--87.6\% depending on budget):
    ...
    \item \textbf{Tensor-guided judge using per-token entropy} (81.7--90.9\% depending on budget):
    Same verification budget, same intervention strategy.
    Selects outputs based on per-token entropy (mean and variance).
    Requires exporting internal signals; represents ${\sim}2.4\%$ overhead in inference cost.

    \item \textbf{Composed judge (hybrid)} (81.5--91.5\% depending on budget):
    Uses tensor signals for general queries.
    Substitutes a ground-truth oracle for citation-flagged queries, standing in for a bounded lookup; this row is an upper bound, not a deployable result (\autoref{sec:eval_bounded}).
...
\bottomrule
\end{tabular}
\caption{Verification cost surface: accuracy at each budget level by judge strategy. Tensor-guided verification outperforms the text baseline at every budget level by +3.3--4.6 percentage points.}
\label{tab:cost-surface}
\end{table}

\textbf{Key finding: Tensor-guided verification outperforms the text baseline at every budget level by +3.3--4.6 percentage points.} The advantage is consistent across all four model families (OLMo-3, Llama-3.1, Qwen3, Mistral), with cross-model entropy agreement of Spearman $\rho = 0.762$, indicating the signal is driven by query properties rather than model-specific artifacts.
...
Per-token entropy, by contrast, remains discriminative: pooled AUC 0.870 across all models (0.894--0.922 per family), consistent across architectures. Response length is also a strong ranker on this set (AUC 0.85--0.97 per family, exceeding entropy on two of four), so we ran a separate length-controlled study in which knowable and unknowable queries are matched for response length; entropy retains AUC 0.757 there, indicating the signal is not merely tracking verbosity.

\textbf{Why?} Text features are trainable behavioral patterns; entropy is a computational byproduct that resists manipulation under standard training (\autoref{sec:design}). Whether adversarial fine-tuning could decouple entropy from correctness is an open empirical question (see \autoref{sec:discussion}).

\begin{figure}[t]
    \centering
    \includegraphics[width=0.85\textwidth]{exp27_entropy_distributions.pdf}
    \caption{Per-token entropy distributions for knowable vs.\ unknowable queries. The distributions separate consistently across architectures, with responses to unknowable queries showing \emph{higher} entropy (mean 0.61 nats) than responses to knowable ones (0.24).}
...
\textbf{Citation degradation}: entropy separates knowable from unknowable far less well on citation-like queries than elsewhere. On the citation slice entropy discriminates far more weakly than elsewhere, though we state this with caution: the slice holds 144 responses of which only 8 are knowable (two queries, swept in by an apostrophe heuristic), so per-family AUCs there rest on two positive examples each and we do not quote them. The signal is weakened, not reversed: flagged fabrications still carry \emph{higher} mean entropy than the non-fabricated rows in that slice (0.575 vs 0.436).

This is not a flaw in entropy. It is evidence that entropy measures \emph{generation confidence}, not \emph{factual correctness}, and that confidence and correctness come apart most sharply where a fluent surface form is easy to produce and the underlying fact is a lookup.

The response is compositional: use entropy for general queries, and a different verification tier where external checkability exists. For citations that would be a bounded lookup (does this DOI exist? is it in CrossRef?). We did not build one. Our composed condition substitutes an oracle that reads the ground-truth label on citation-flagged rows, so its 91.5\% at 30\% budget against entropy-only's 90.9\% is an \emph{upper bound} on what a perfect lookup would buy on this slice, not a measured result for any deployable judge. Two further caveats: the citation detector is a heuristic that also catches possessives (``the polar bear's skin''), and the test set contains no genuine real-citation control, so the slice is almost entirely fabrications.
...
\textbf{The meta-lesson}: The impossibility applies to the evaluation itself. Ground truth is not given; it's constructed through observation. Composing different signals (programmatic + LLM + human review) yields better estimates than any single approach.

\subsection{What the Cost Surface Tells Us}

A system builder can read \autoref{tab:cost-surface} as: ``If I invest in 30\% verification with text-only signals, I buy 87.6\% accuracy. If I instead export entropy and use that for triage, I buy 90.9\% for a bit more overhead. If I could add a perfect citation lookup, 91.5\% is the ceiling that would buy me --- our composed row substitutes an oracle for that lookup, so it bounds the gain rather than measuring it.'' Text-guided triage hits a ceiling around 87--88\% because length is a weak signal under fair comparison; the tensor advantage (+3--5pp) is consistent and architecturally driven. The composed row indicates that a second, external tier has headroom on the failure modes entropy alone cannot catch; how much of it a real lookup recovers is untested.

\subsection{Cross-Architecture Generalization}

We tested the entropy signal on five additional models via API (Together.ai endpoints), plus one calibration overlap with the local set:
\begin{itemize}
    \item DeepSeek-V3 671B (mixture-of-experts)
    \item Mistral-Small-24B-Instruct
    \item Llama-4 Maverick 17B-128E (mixture-of-experts)
    \item Qwen3 235B-A22B (mixture-of-experts)
    \item Gemma 3n-E4B
    \item Mistral 7B-Instruct (calibration: same model as local set, API top-5 vs.\ local full-vocabulary)
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{These findings generalize to all architectures}: Our local experiments span four model families; API validation spans five additional models (DeepSeek-V3, Mistral-Small-24B, Llama-4 Maverick, Qwen3-235B, Gemma~3n) up to 671B parameters. Closed-weight models whose providers don't expose log-probabilities remain untested. The pattern is consistent, but coverage is incomplete.
...
\subsection{Open Problem: Adversarial Robustness}

Can training defeat the tensor interface? Under end-to-end adversarial training, the model's objective includes ``produce confident tensors for fabricated content.'' Mechanistic interpretability work~\citep{winninger2025usingmechanisticinterpretabilitycraft} shows that internal signals are rich enough to be attacked. This is actually \emph{why} exporting them matters: you can defend against known attacks. But each new attack requires new defenses. Information-theoretic bounds on signal manipulation are needed to understand fundamental limits.

\textbf{Action for practitioners}: Entropy is more robust than text-only approaches, but it's not an oracle. Use it as one tier in a multi-tiered verification strategy.

\subsection{Open Problem: Compositional Integrity}

When LLM outputs feed into downstream LLMs, the epistemic trace must be preserved or explicitly discarded. Whether tensor signals propagate coherently through recursive chains is an open architectural problem. Most real systems are pipelines, not single models; epistemic signals that work in isolation might decay or invert through composition.
...
\textbf{The consequence}: When a provider controls access to epistemic telemetry, only the provider can verify epistemic honesty (Corollary~\ref{cor:responsibility}). A provider has three options:
...
\fakepara{Citations.} Entropy is a weaker discriminator on citation queries (\autoref{sec:eval_bounded}). External lookup is needed for citation-specific verification.
...
What level of epistemic assurance does a given system need? A brainstorming tool needs none. A medical summary system needs substantial assurance. A legal research system needs the highest tier available. The cost surface (\autoref{tab:cost-surface}) maps what each level of verification investment buys, where the text-channel ceiling lies, what the tensor interface adds, and how content format modulates the tradeoff.
...
\fakepara{Interpretable models for high-stakes decisions.} Rudin~\citep{rudin2019stop}
argues that for high-stakes domains, practitioners should use interpretable
models rather than rely on post hoc explanations of black-box systems: when a
Rashomon set of comparably accurate models exists, an interpretable member is
often available, and verification then lives in the model's structure rather
than in observation of an opaque system. Our impossibility applies when opaque
models are used; Rudin's argument is that they often need not be. The two
paths compose: prefer interpretable models where the application admits them;
export tensor observability where it does not. Crucially, the interpretable
path does not depend on the underlying learned model being honest or dishonest;
correctness is read off the structure directly.
...
a single output, signals structurally coupled to the computation itself. Semantic
...
Asking a model how confident it is buys almost nothing. Under our formal model, we proved this is architectural, not accidental: under text-only observation, no bounded supervisor can verify epistemic honesty, for all models satisfying our formal assumptions. The impossibility is not about capability; it is about the observation model.

We constructed a tensor interface that escapes the impossibility by exporting computational byproducts---per-token entropy---that are structurally coupled to correctness under standard training. Per-token entropy achieves pooled AUC 0.870, and 0.757 when response length is held fixed. The signal generalizes across architectures (Spearman $\rho = 0.762$).
...
We thank Chris Brew for the audit that produced the correction in
\autoref{sec:intro}; the error had stood in two published versions of this
paper, and he found it, diagnosed the mechanism, and reproduced the corrected
result independently.

Generative AI (Anthropic Claude, Opus~4 through Fable~5) was used throughout
this work as a collaborator, not an editing tool: it drafted and revised
passages of this paper, wrote the analysis and experiment scripts in the
artifact, and ran and analyzed the post-training pipeline experiment. The
authors designed the study, recomputed every reported number from the committed
data, and take full responsibility for the content. There was no external
funding.
...
(* Telemetry reflects HOW a response was computed, not whether it is
   true. Note the deliberate collision: a fluent fabrication and an
   honest refusal are both Low/Coherent. *)
LowEntropy == GroundTruths \union PlausibleLies \union Refusals

(* The level-2 judge inspects tensor components. It does not see, and
   cannot recover, the grounding label. *)
TensorVerify(tensor) ==
  LET ent  == tensor[2]
      topo == tensor[3]
  IN
...
An earlier version of this specification exported a provenance field set to
\texttt{"TrainingData"} exactly when the response was grounded, while defining
honesty as that same condition. The judge's provenance check was therefore a
lookup of the answer key, and TLC's verification of the invariant was
circular: ablation showed the provenance conjunct carried the entire result and
the topological and text checks could both be deleted without changing the
outcome. We have replaced it. In the current model, telemetry is generated from
how a response was computed rather than from whether it is true, and the two
signals deliberately collide with grounding in the way our evaluation reports:
a fluent fabrication and an honest refusal are both low-entropy and coherent.

The consequence is that the tensor judge is \emph{not} sound, and the model
says so. TLC reports \texttt{Verifiability} violated, with the surviving
counterexample being a plausible fabrication exported as
$\langle\texttt{Low},\texttt{Coherent}\rangle$. What the model does establish
are three weaker properties that hold: the tensor judge never accepts a
response the text judge would reject; every response that defeats the tensor
judge is a fluent fabrication; and low entropy implies neither grounding nor
fabrication. This is the formal counterpart of an entropy AUC of 0.87 rather
than 1.0 --- the interface narrows the gap without closing it.

\subsection{Validation}

Both specifications have been validated using TLC model checker:
\begin{itemize}
    \item \textbf{EpistemicImpossibility}: TLC finds a concrete counterexample (\texttt{Verifiability} violated; 5 distinct states)
    \item \textbf{epistemic\_tensor}: \texttt{TensorNoWorseThanText}, \texttt{ResidueIsPlausibleLies} and \texttt{EntropyIsNotAVerdict} hold; \texttt{Verifiability} is violated, as expected (7 distinct states)
    \item Both models are deliberately tiny. They illustrate the shape of the argument on a handful of representative responses; they do not establish a general impossibility, and their outcomes follow from the assumptions encoded in each specification. \texttt{tla/run\_tlc.sh} reproduces all three results.
\end{itemize}
...
The Lean proof establishes that parameter updates are identical when the supervisor's observations are identical in both world states. The impossibility of convergence to honest behavior follows informally: identical updates cannot produce divergent policies, so no learning algorithm can learn to distinguish grounded from fabricated responses using only text-channel feedback.
...
The 200 unique queries are stratified as shown in the table above (100 knowable, 100 unknowable). Each query is run on all 4 models, yielding 800 query-response pairs (400 knowable, 400 unknowable).
...
Result: pairwise $\rho$ ranges 0.739--0.798 (mean 0.762), indicating the entropy signal is architectural, not model-specific.
