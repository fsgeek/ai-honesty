...
\begin{itemize}
    ...
    \item \textbf{Citations}: This case is instructive. Fabricated citations sound \emph{more} confident than real ones because they're fluent inventions requiring no retrieval. Real citations require looking up specific details, introducing uncertainty. The signal inverts: the judge designed to catch fabrication actually flags truthful citations more aggressively. This is not a bug in citation verification; it's a feature of how entropy works. It's also a reminder that different failure modes require different signals.
...
These are byproducts of what the model actually computes. They're harder to fake because faking them means changing the computation itself, which affects correctness.

If a supervisor has access to:
\begin{itemize}
    \item \textbf{Per-token entropy}: How uncertain was the model's computation while generating this token?
    \item \textbf{Attention patterns}: Were the attention patterns coherent, or scattered across unrelated tokens?
    \item \textbf{Log-probabilities}: What probability did the model assign to the tokens it generated?
\end{itemize}

Then the supervisor has signals that correlate with whether the generation is grounded or fabricated. Not perfectly, but consistently. And critically: the model cannot learn to manipulate these signals through RLHF or instruction tuning without degrading its own correctness.

This is the tensor interface: export the telemetry alongside the text, giving supervisors access to signals that are structurally harder to fake.
...
\textbf{Why?} Length, hedging, and citation patterns are trainable---the model can learn to match format conventions through RLHF without actually knowing the difference between grounded and fabricated content. Entropy is different. The model cannot learn to produce confident distributions (low entropy) while simultaneously outputting wrong answers, because confidence and token choice are computed together.
...
This is a governance decision, not a technological one. It determines who bears the cost of verification, whether that cost is visible or hidden, and whether verification can be externally audited. Different choices have different consequences for trust, accountability, and liability.

\subsection{The Industry Trend: Toward Greater Opacity}

Here is an uncomfortable observation: the models where epistemic verification is hardest are precisely the ones providing the least epistemic telemetry.

Early language model APIs (GPT-3, 2020) exported log-probabilities and per-token likelihoods. These fell outside the text-only impossibility class and enabled external verification. Current APIs from major providers often don't. Newer reasoning models (o1, etc.) typically export nothing beyond text, making them fully subject to the impossibility results.

This is not a technical limitation. Log-probabilities require negligible additional computation to export. The choice to withhold them is a governance decision: a provider deciding that verification responsibility will remain internal, hidden from users and third-party auditors.

The consequence: as models become more capable, the opacity deepens. The frontier models—the ones deployed in high-stakes settings—provide the least observability. A medical system, a legal research tool, a financial advisor built on the latest frontier model is subject to the full impossibility results. Its creators cannot externally verify whether it is honest. They are entirely dependent on the provider's internal verification discipline.

This is not a research problem. It is a policy problem. It determines whether epistemic verification is externally auditable or internally opaque, whether third-party assurance is possible or excluded by design.
...
\fakepara{Citations.} Fabricated citations are generated fluently (low entropy) because the model invents plausible formats without retrieving specifics. Real citations require looking up details, introducing uncertainty (higher entropy). Entropy inverts on citation queries. External lookup is needed for citation-specific verification.
