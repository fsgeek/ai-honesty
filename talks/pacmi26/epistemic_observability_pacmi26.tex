...
\begin{frame}{Tensor-guided verification wins at every budget}
  \begin{columns}[T]
    ...
    \begin{column}{0.42\textwidth}
      ...
      \vspace{0.4em}
      \pp{+3.3}, \pp{+4.6}, \pp{+3.3} over the best text signal at
      10, 20, and 30\% verification.

      \vspace{0.4em}
      Every family improves over no judge. At 10\%: OLMo
      69.5\,$\to$\,75.1, Llama 82.5\,$\to$\,88.1, Qwen 87.0\,$\to$\,91.7,
      Mistral 64.0\,$\to$\,72.0.
    ...
    \end{column}
  \end{columns}
  \note{9:30. Headline, then the per-family line. Do not recite the
  non-claims here; they are in the paper. If asked about length as a
  detector, the next slide is the answer.}
\end{frame}

%------------------------------------------------------------------------------
\begin{frame}{Two signals, different items}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \textbf{Word count is a strong text signal.} It ranks knowable
      against unknowable at AUC 0.85--0.97 per family. The tensor's
      budget advantage is not a better detector. It comes from
      \emph{which items} each signal selects for verification.

      \vspace{0.5em}
      \textbf{And it is a signal training can move.} Length and hedging are
      surface regularities; a model can be trained to change them without
      changing the computation beneath. The entropy trace moves only when
      the computation does.
    \end{column}
    \begin{column}{0.5\textwidth}
      \textbf{Post-training does not erase it.} Four public OLMo-3
      checkpoints: base, SFT, DPO, RLVR.
      \begin{itemize}
        \item Knowable-query entropy falls 1.18\,$\to$\,0.18 nats.
        \item Fabrications that survive stay high: 1.49\,$\to$\,0.98.
        \item The separation \emph{widens} through every stage.
        \item Refusals on fabrication prompts rise 0\%\,$\to$\,56\%.
      \end{itemize}
      \vspace{0.3em}
      Post-training improves the testimony and leaves the telemetry usable.
    \end{column}
  \end{columns}
  \note{10:45. This is the reason to want the telemetry when text is this
  good: different items, and a signal the model cannot move separately from
  its computation. AUC figures and the probe-set caveat are in backup.}
\end{frame}
...
\begin{frame}{The ask}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      Log-probabilities were on early completion endpoints. They are absent
      from the newest reasoning models, the systems whose output is hardest
      to verify.

      \vspace{0.5em}
      When only the provider can see the telemetry, only the provider can
      verify. Everyone downstream pays to reconstruct what the model already
      computed.
    \end{column}
    \begin{column}{0.5\textwidth}
      \textbf{It works through an API, too.} Top-5 log-probabilities from
      five serverless endpoints, 4B to 235B parameters including two
      mixture-of-experts models, separate knowable from unknowable at AUC
      0.65--0.88.
    ...
    \end{column}
  \end{columns}
  \note{12:00. End here. If short on time, drop the runtime slide, not
  this one.}
\end{frame}
...
\begin{frame}{Backup: what the paper does not claim}
  \small
  \begin{itemize}
    \item That entropy is the better \emph{detector}: length ranks at AUC
      0.85--0.97 and beats entropy (0.87--0.92) on Llama and Mistral. On
      Mistral at 30\% budget the length judge wins by 2.2\,pp.
    \item That the composed judge is achievable: it reads the ground-truth
      label for citation queries, so its 91.5\% is the headroom a perfect
      lookup would buy.
    \item That the post-training ceiling is 1.00: on the 48 probes it is;
      counting the true-but-implausible category gives 0.63\,$\to$\,0.81--0.87.
      The widening is robust, the ceiling is not.
    \item That the tensor is unfakeable under adversarial training, that it
      survives composition in recursive pipelines, or that the design is
      optimal.
    \item Closed-weight models whose providers export no log-probabilities
      remain untested.
  \end{itemize}
\end{frame}
