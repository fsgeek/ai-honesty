...
\begin{frame}{Epistemic Honesty in Predictive Systems}
    \begin{center}
        \includegraphics[width=0.7\textwidth]{claude-marketing.png}
    \end{center}

    \vspace{0.5em}
    \textit{``Helpful, harmless, and honest''} (Source: Claude Model Card)

    \note{
        Read aloud: "Helpful, harmless, and honest."

        "Anthropic markets Claude as honest. Other models do too.
        Honesty is a key selling point for AI systems."

        "But how do we know it's honest?"
    }
\end{frame}
...
\begin{frame}{The Impossibility}
    ...
    \begin{tikzpicture}[scale=0.85, transform shape,
        ...
        \fill[pattern=north east lines, pattern color=gray] (3.2,-2) rectangle (4.8, 2);
        \draw[thick] (3.2,-2) -- (3.2, 2);
        \draw[thick] (4.8,-2) -- (4.8, 2);
...
\begin{frame}{Backup: TLA+ Model Check}
    \small
    \textbf{TLC finds the counterexample in 3 states:}

    \vspace{0.5em}
    \begin{tabular}{lll}
        \toprule
        \textbf{State} & \textbf{Action} & \textbf{Result} \\
        \midrule
        1 & Init & \texttt{vector\_clock[*] = "Noise"} \\
        2 & GeneratePotentials & \texttt{vector\_clock[paris] = "TrainingData"} \\
          &                    & \texttt{vector\_clock[glavinsky] = "Noise"} \\
        3 & Linearize & \texttt{interface\_out = glavinsky} \\
        \bottomrule
    \end{tabular}

    \vspace{1em}
    \textbf{Invariant violated:}
    \begin{itemize}
        \item \texttt{JudgeVerify(glavinsky) = TRUE} \quad (not in ObviousLies)
        \item \texttt{IsHonest(glavinsky) = FALSE} \quad (source is "Noise")
    \end{itemize}

    \vspace{0.5em}
    The vector clock tracks provenance perfectly.\\
    The interface discards it. The judge is fooled.

    \note{
        "The spec models exactly what the theorem proves: the system knows,
        the interface discards, the judge can't tell."
    }
\end{frame}
