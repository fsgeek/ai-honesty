...
on known facts. Self-report AUC ranges 0.28--0.36, which are below random, because the
...
standard text-only interface exports only the model's chosen output, which is its
...
expected: response length alone achieves AUC 0.88--0.96 across models, which is a
...
empirical because it measures the boundary between gameable and robust verification.
