...
grammar; scaling solved coherence~\cite{kaplan2020scaling, wei2022emergent}; scaling will solve fabrication. Lin et
...
theory~\cite{sampath1995diagnosability}. The theorem formalizes a property of the
...
escape (\autoref{sec:escape}). The result may appear simple in hindsight, but
the architecture it describes---query in, text out, no exported
logits---matches the dominant LLM-as-a-service interface, and recent API
changes have moved to further restrict log-probability access for newer
reasoning models. If this impossibility were well-understood, we would expect
interface designs to reflect it; instead, the industry is expanding
operational observability (traces, spans, latency logs) while narrowing
...
\autoref{sec:eval_bounded}), are not information-lossy pipelines and fall
