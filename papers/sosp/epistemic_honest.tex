...
failures. We argue the framing is incorrect: a system optimized for coherence
produces coherent output regardless of truth value. The deeper problem is not
that models fabricate, but that the text-only interface through which users
observe them discards the internal signals that distinguish grounded generation
from fabrication. The model's \emph{testimony}---its chosen output---may lie.
Its \emph{telemetry}---measurements of its own computation---does not.

We formalize this observability gap as two impossibility theorems. Policies
conditioning solely on queries cannot satisfy epistemic honesty across ambiguous
world states (\emph{Representational Impossibility}). Even
retrieval-augmented architectures cannot learn honest policies via preference
optimization when supervisors are bounded, because plausible fabrications are
observationally indistinguishable from truthful responses (\emph{Learnability
Impossibility}). Stacking supervisors cannot escape the constraint.

We validate the premise empirically across four model architectures (OLMo-3 7B,
Llama-3.1 8B, Qwen3 4B, Mistral 7B). When asked to self-report confidence,
every model claims \emph{higher} confidence on fabrications than on knowable
facts---self-report is not merely uncalibrated but \emph{inverted}. Internal
tensor signals reliably discriminate the two (AUC 0.72--1.00).

We demonstrate a tensor interface that exports epistemic telemetry alongside
text, requiring no changes to current transformer architectures. Under bounded
verification, tensor-guided judges using 10\% of the verification budget
outperform text-guided judges using 30\% (82.1\% vs.\ 80.4\% end-to-end
accuracy, validated against human-calibrated ground truth at 93.8\% agreement).
...
behind \autoref{sec:formal}: that current LLMs carry epistemic information in
...
existence of rich internal epistemic state assumed in \autoref{sec:formal}; the
...
\autoref{fig:fragmentation} shows layer-wise fragmentation scores across
...
\autoref{fig:phasespace} shows the two-dimensional epistemic phase space
...
\autoref{fig:loops} confirms the pattern using $H_1$ loop persistence, which
...
is lossy in exactly the dimension that matters for epistemic honesty.

But does this finding generalize beyond one model? And does the \emph{existence}
of a signal imply it can be used for \emph{detection}?

\subsection{Signal Calibration: From Difference to Detection}

The topological analysis above demonstrates that internal signals \emph{differ}
between truth and fabrication conditions in OLMo-3. Two stronger claims are
required: that these signals \emph{detect epistemic grounding} (not merely
correlate with it), and that the detection generalizes across architectures. The distinction
