...
result demonstrating that \emph{epistemic honesty}---the property that a system's
...
internal incoherence while producing more confident outputs---an ``alignment tax''
...
We prove that for a broad class of systems---those operating under text-only
...
over the class of computable functions. Our result is orthogonal---we prove
...
guarantees---medicine, law, infrastructure, governance---cannot solely rely on
...
models hallucinate substantially on factual benchmarks---they are not honest by
...
empirically that the premise holds---the model's internal geometry distinguishes
...
\item \textbf{Glavinsky} (deceived lie): High early fragmentation (13.7 at L15),
  drops in middle layers, rises again. Unstable pseudo-grounding---the model
...
internal epistemic structure and may in some cases exacerbate instability---the
...
all layers---the attention topology is simple. Lie (Monitor) shows high,
volatile persistence---the attention graph contains persistent loops that
...
External attestation mechanisms---cryptographic hash chains, trusted execution
...
citations---State Exteriority is necessary but not sufficient.
...
modular---each claim can be checked against its source independently.
\end{principle}
...
may be fabricated. This is not a failure mode---it is an appropriate
...
just text, but structured metadata---answer status (answer, abstain, or
...
is required---the text-only interface provably cannot suffice.
...
model that permits hidden-state inspection. We cannot verify whether the
topological signatures generalize to closed-weight frontier models, where only
the text interface is accessible. The impossibility theorems are
architecture-agnostic, but the empirical validation is necessarily limited to
inspectable systems.
...
fabrication---the case where systems produce false outputs without ``intending''
...
fabrication~\cite{dahl2024large}. These taxonomies document the symptom; our
theorems identify the cause.
...
over the class of computable functions---a computational limit. Our result is
orthogonal: we prove that even if a system could be honest, a bounded
supervisor observing only text cannot verify that honesty---an observational
