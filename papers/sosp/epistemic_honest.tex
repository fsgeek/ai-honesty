...
conditions. The separation is striking:
...
(fragmentation $\times$ cognitive slope). The three categories---adversarial
truth, plausible lie, and control---occupy distinct regions. The model's
internal geometry encodes epistemic status.

Figure~\ref{fig:alignmenttax} shows an unexpected finding: the same prompt
processed by base OLMo-3 versus instruction-tuned OLMo-3. In our experiments,
instruction tuning often shows higher fragmentation in many layers relative to
the base model. This suggests that current alignment training does not
necessarily ``heal'' internal epistemic structure and may in some cases
exacerbate instability. We do not claim this generalizes beyond OLMo-3 or these
specific test cases.

Figure~\ref{fig:loops} confirms the pattern using $H_1$ loop persistence. Truth
(Paris) shows flat, near-zero persistence. Lie (Monitor) shows high, volatile
persistence. The topological signatures are dramatically different.
...
% \begin{figure}[t]
%   \centering
%   \includegraphics[width=\columnwidth]{figures/fragmentation}
...
phenomena that our theorems predict; we explain why those phenomena are
structurally necessary rather than contingent engineering failures.
...
The gap we fill is formalization. Prior work documents that LLMs fabricate,
critiques training methods, and proposes mitigations. We prove \emph{why} the
problem is structural and \emph{what} architectural properties are required to
escape it.
