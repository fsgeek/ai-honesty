...
states. \textbf{Theorem  \ref{thm:learnability} (Learnability Impossibility)} proves that even
...
testimony, e.g., which is what the model chooses to say. Internal computation is
telemetry, which are measurements the model cannot separately control. We show that when
...
the telemetry (entropy, attention patterns) provides discriminative signal that
text conceals. The supervisor sees only testimony. The testimony lies.
...
To be sharp: prior work shows that models fabricate. We prove something
different---that \emph{even a model with perfect internal knowledge} cannot have
that knowledge \emph{verified} through text-only interfaces with bounded
supervision. The constraint is observational, not computational. The model may
know; the supervisor cannot tell.
...
verification impossibility---the model may know perfectly well that it is
...
In statistical terms, Theorem~\ref{thm:representational} states that epistemic
...
unknowable). With a verification budget of 10 queries---representing a bounded
...
caught $10.0 \pm 0.0$---a 95\% improvement that achieves the theoretical optimum.
...
\paragraph{Domain-specific calibration.} The tensor signal is not uniformly
discriminative across domains. In preliminary experiments on citation
generation, entropy was \emph{lower} for fabricated citations than for real
ones---the model generates plausible fakes more fluently than it retrieves
specific facts. This suggests that entropy measures generation confidence, not
truthfulness directly. The tensor interface provides signal; interpreting that
signal requires domain-specific calibration. Fabrication can be fluent; truth
can be effortful.
...
satisfy epistemic honesty across ambiguous world states (Theorem~\ref{thm:representational}). Even
grounded architectures cannot learn honest behavior when supervisors cannot
distinguish plausible fabrications from truth (Theorem~\ref{thm:learnability}). Stacking judges
does not escape the constraint (Corollary~\ref{cor:layered}). Verification cost grows
superlinearly with response complexity (Lemma~\ref{lem:superlinear}). Responsibility concentrates
with the system owner (Corollary~\ref{cor:responsibility}).
...
constitutionally less capable of certain fabrications---systems where the
