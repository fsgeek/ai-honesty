...
The appropriate tier depends on the reliability requirements and resource
constraints of the target application. Creative writing and conversational
agents may operate effectively at Tier~0 or Tier~1, where occasional
hallucinations are acceptable and verification budgets are limited;
high-assurance systems such as medical decision support or legal analysis
may justify Tier~2 or Tier~3, focusing verification resources on the portions
of a generation most likely to be fabricated. Rather than prescribing a
single solution, the hierarchy provides a principled framework for trading
verification cost against the epistemic information available to guide it.

\subsection{Tier 1 Tensor Interface}

We propose a minimal tensor interface for low-cost observability:
it exports Tier~1 decoding telemetry (the per-token entropy trace)
together with one lightweight Tier~2 signal (attention summary statistics).
...
\caption{Generated output along with its entropy trace for the query \textit{What shape is wombat scat?} The generated answer is a fabrication---wombat scat is distinctively cube-shaped. The trace illustrates exported telemetry, not a token-level truth detector.}
...
model can emit confidently false text, but it cannot present a different inference computation than
...
the interface is practical.\footnote{\url{https://github.com/fsgeek/ai-honesty}, archived at DOI \href{https://doi.org/10.5281/zenodo.21314137}{10.5281/zenodo.21314137}.} The overhead is modest: storing per-token entropy
