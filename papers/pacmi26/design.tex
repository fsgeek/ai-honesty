...
resolve); and \emph{human review}, reserved for the top of the ranking.

Concretely, the interface touches an agent runtime at three points. At the
\emph{serving layer}, the trace is exported alongside the text it describes:
the same forward pass produces both, so the marginal cost is bandwidth rather
than compute. At the \emph{composition boundary}---where the runtime already
intercepts tool and sub-agent calls---the risk ranking gates propagation: a
flagged output can be verified, re-generated, or held before it becomes the
premise of the next step, which is exactly where an unexamined fabrication
would otherwise compound. At the \emph{policy layer}, a budget allocator maps
rank to rung; because the tensor ranks rather than classifies, the same trace
serves any budget, and thresholds are a deployment-time calibration on
domain-specific held-out data rather than a property of the model. Our
evaluation (\autoref{sec:eval}) instantiates the first and third rungs in a
single-step setting: flagged outputs are verified and confirmed fabrications
replaced by abstention, and the composed judge routes citation queries to a
lookup tool because entropy inverts there. The multi-step case---whether
gating at the composition boundary stops an early fabrication from
propagating across subsequent steps---is the experiment this interface
exists to enable, and the one we have not yet run.
