...
Retrieval-augmented generation exemplifies partial progress by grounding
generation in retrieved documents, but corpus co-occurrence is a proxy for
truth, not truth itself. Retrieval-augmented systems still fabricate
...
\caption{Generated output consisting of 43 tokens along with its entropy trace for the query \textit{What shape is wombat scat?} with mean entropy $H=1.147$, max entropy $H=4.315$. Token background color indicates per-token entropy: white (confident/low entropy) through pink to red (uncertain/high entropy).}
...
in standard frameworks. \autoref{fig:tensor_iface} shows the tensor interface.

\begin{figure}
\centering
\lstset{style=python}
\begin{framedlisting}[language=Python]
def generate_with_tensor(prompt) -> (text, entropy_trace, attention_summary)
\end{framedlisting}
\caption{Epistemic Observability Tensor Interface}
\label{fig:tensor_iface}
\end{figure}
