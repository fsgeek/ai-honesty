...
OLMo-3 publishes every post-training checkpoint, so we ran the same probes
through its base, SFT, DPO, and RLVR stages (greedy decoding; 16 knowable and
32 fabrication prompts).
Post-training sharpens everything, but not evenly: mean entropy over fabricated
content fell from 1.49 to 0.92 nats while over knowable answers it fell from
1.18 to 0.18, so the separation the interface relies on \emph{widened}
(AUC 0.67 $\rightarrow$ 0.998).
Over the same stages the disclaimer rate rose from 0\% to 56\%, and hedged
fabrications were computed with no more uncertainty than unhedged ones:
post-training installs the hedge independently of the state it purports to report.
