...
\citeauthor{xu2024hallucination}~\cite{xu2024hallucination} proves that fabrication is
...
For example, checking a citation against a database costs latency; running a second model as a judge costs compute.
...
As a proof of concept, we design a tensor-generation interface that exposes Tier~1 epistemic telemetry, which includes the per-token entropy,
and demonstrate that tensor-guided judges consistently outperform text-only baselines across all verification budgets, 
