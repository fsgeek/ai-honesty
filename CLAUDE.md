...
- **Response length is a STRONG text baseline**: raw word count scores AUC 0.85-0.97 per family and beats entropy on two of four (Llama 0.949 vs 0.922, Mistral 0.969 vs 0.905). The tensor advantage in the budget curve comes from *which items* entropy selects, not from a uniformly better detector. Do not claim entropy dominates length.
  - Measure on **raw word count**, not the `length_score` column — that column is a pre-binned derived value (0.8 for a 1-word response) and scoring it gives a misleading 0.45-0.67.
- **The composed judge is an oracle, not a lookup**: `fig3_budget_curve.py` scores citation-flagged rows from the ground-truth `is_knowable` label. The 91.5% figure is an upper bound on what a perfect citation lookup would buy. The "citation entropy inversion" story is also unsupported — flagged fabrications have *higher* entropy (0.575 vs 0.436), and the 8 "real citation" rows are apostrophe false positives.
